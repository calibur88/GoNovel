...
| 级别 | 含义 | 覆盖的错误类型 |
|---|---|---|
| `error` | **结构与声明写错** —— 必须改文件，且会让整块内容无法解析 | 缺少 frontmatter、`gnd_type` 缺失／非法、未知关键字、关键字重复、同级目录类型相同、读取失败 |
| `warning` | **取值、引用或登记失败** —— 文件能用，只是取值不理想、或指向／登记的东西不可用 | 空变量名、变量重复、导入路径错误、导入目标不存在、导入类型错误、文件不存在、主页类型错误、封面路径非法、封面图片不存在 |
...
6. **清理**：删除仓库里的 `小说项目/`、`调试样例/` 与 `assets/` 三个目录，再删掉 `.obsidian/plugins/go-novel/data.json`，即退回干净仓库。
