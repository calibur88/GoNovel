...
- **主页管理视图**（Ribbon 图标「gn 主页管理」，点击 toggle 开关）：顶部固定标题、下接可定制管理语隔离条，主体为卡片网格；卡片按调色板上色（灰显卡片统一灰底），卡内三行文字居中展示路径与创建／修改时间；点击卡片打开对应看板；底部四按钮：刷新 / 清理 / 登记 / 删除（「登记」只从库中**已有**的 `.gnd` 里挑，插件不创建文件），其中**刷新以 `data.json` 为真相源**（重读磁盘设置后重扫重渲，用于外部改动后手动同步）；
- **小说项目主页（看板）**：独立视图，展示欢迎词区（`[欢迎词]`）与「我的作品」卡片区（`**WHERE**` 字段）；作品卡片使用与管理视图相同的调色板与规则（相邻不同色），配色存于 `projectColors`；卡片顶部渲染作品页 `gnd_image` 指定的封面图；
...
- **调试信息视图**（`gonovel-debug`）：右侧边栏，由设置里的「调试信息开关」控制开合（dev 构建默认开启、正式版默认关闭）；上半留空、下 1/3 为日志区，内容分两块——`.gnd` 诊断（**只诊断 `data.json` 里已登记的文件**，按「级别 + 错误类型 + 文件」聚合成行：点线 + 条数，整块按**产生时间倒序**，不按级别重排）与运行日志（`登记主页` / `删除主页` / `清理完成` / `索引完成` / `刷新完成`，最新在顶）；**级别按「谁的问题」分三档**：`error` = 结构与声明写错（frontmatter / `gnd_type` / 关键字、同级目录类型相同、读取失败），`warning` = 取值、引用或登记失败（变量、导入路径／目标、封面图片、主页登记态），`info` = 运行期信息（`**WHERE**` 取空、运行日志）；底部按钮「刷新 / 清空 / 折叠 ▲」，其中刷新＝只读重跑登记 `.gnd` 的解析，不写 `data.json`、不改内容；
...
> **版本兼容性**：当前版本 0.5.0 与 0.1.0 之前的设计**不兼容**——`.gnd` 打开方式由自定义视图改回宿主原生 markdown，设置结构收敛为 `homePaths` / `managerNote` / `debugEnabled` / `homeColors` / `projectColors`，需重新登记主页；**0.1.0 → 0.5.0 各版本之间完全兼容**（`data.json` 结构不变，旧字段 `cardColors` / `workColors` 自动迁移）。
...
1. 在 `data.json` 中登记主页路径：
   ...
   `homeColors` / `projectColors` 都可以留空，插件会按「相邻不同色」自动补齐。旧字段 `cardColors` / `workColors` 会在读入时自动迁移。
...
- `gnd_type`：`home` / `page` / `cache` / `data`；仅 `home` 渲染为看板；
- `gnd_image`：**仅 `page` 有效**的封面字段，值为**相对 vault 根**的库内图片路径（写 `assets/cover.png`，**不写 vault 名**，也不写盘符；含 `..` 视作非法）。路径非法或图片不存在时，封面退回空槽并在调试框记一条 `warning`（挂在声明封面的那个 `page` 上）；
- `[欢迎词]`：看板欢迎词，兼容 `[WELCOME]`，缺失或为空则整块不渲染；
- `**SELECT**`：导入的作品文档，路径相对于当前文档所在目录、必须多级、禁止 `..`，目标须为 `page` 类型；
...
| 入口 | 登记 | 删 | 改文案 | 刷新 | 清理 |
|---|---|---|---|---|---|
| 主页管理视图 | ✅ 登记 | ✅ 卡片 ✕（进回收站） | — | — | ✅ 两步确认 |
| 设置面板 | ✅ + 登记主页路径 | ✅ −（只移出登记，不动文件） | ✅ 管理说明 | ❌ | ❌ |

视图是完整操作台（含破坏性操作，需两步确认），设置面板是最小编辑入口（登记／移除 + 文案兜底）。两处的「登记」都只从库中已有的 `.gnd` 里挑，**插件不提供创建文件的入口**。任一边改动都经同一份 `homePaths`，另一边随即同步重渲染；主页路径被移除后，对应的看板 leaf 会直接关闭（不保留空壳）。
